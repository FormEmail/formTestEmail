...
phoneInput.addEventListener("input", () => {
  let soloNumeros = phoneInput.value.replace(/\D/g, "");
  ...
  // Si quieres mantener espacios cada 3 dígitos
  phoneInput.value = soloNumeros.replace(/(\d{3})(?=\d)/g, "$1 ");
});
...
form.addEventListener("submit", (e) => {
  ...
  } else { // Enviamos todos los datos
    const formData = {
        name: nameInput.value.trim(),
        phone: phoneInput.value.trim(),
        email: emailInput.value.trim(),
        proyectNumber: orderNumber.value.trim(),
